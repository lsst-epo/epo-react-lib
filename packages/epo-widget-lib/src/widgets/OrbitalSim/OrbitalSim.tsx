...
function OrbitalSim() {
  ...
  const [currentSwappableOrbit, setCurrentSwappableOrbit] = useState({
    neos: [{ Principal_desig: "" }],
  });
  ...
  const updateSwappableOrbit = (id: string) => {
    const neo = orbits?.neos?.find(
      ({ Principal_desig }: Neo) => Principal_desig === id,
    );
